...
export const createNewProduct = async ({ data, images, useLocalStorage = false, tenantId = 'default' }) => {
    if (!db) {
    ...
    try {
        ...
        const productData = {
            ...
            sku: sanitizeInput(data.sku?.trim() || ''),
...
export const updateProduct = async ({ id, data, newImages = [], useLocalStorage = false }) => {
    ...
    try {
        ...
        if (data.sku) updateData.sku = sanitizeInput(data.sku);
        ...
        if (data.sizes) updateData.sizes = Array.isArray(data.sizes) ? data.sizes.map(s => sanitizeInput(s)) : [];
        if (data.colors) updateData.colors = Array.isArray(data.colors) ? data.colors.map(c => sanitizeInput(c)) : [];
